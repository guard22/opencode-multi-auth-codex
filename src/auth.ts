...
import { addAccount, updateAccount, loadStore, mutateStore } from './store.js'
...
const tokenRefreshes = new Map<string, Promise<AccountCredentials | null>>()
...
async function refreshTokenWithLock(alias: string): Promise<AccountCredentials | null> {
  let initialAccount: AccountCredentials | undefined
  try {
      ...
      try {
        ...
        if (!tokenRes.ok) {
          ...
          if (tokenRes.status === 401 || tokenRes.status === 403) {
            let newerAccount: AccountCredentials | null = null
            try {
              mutateStore((store) => {
        ...
        let result: AccountCredentials | null = null
        mutateStore((store) => {
          const current = store.accounts[alias]
          if (!current) return
          if (
...
export function refreshToken(alias: string): Promise<AccountCredentials | null> {
